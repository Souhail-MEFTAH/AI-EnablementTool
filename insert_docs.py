...
def main():
    ...
    parser.add_argument("url", nargs="?", help="URL to crawl (regular, .txt, or sitemap)")
    ...
    parser.add_argument("--clear-db", action="store_true", help="Clear the ChromaDB directory before inserting")
    ...
    if args.clear_db:
        if os.path.exists(args.db_dir):
            print(f"[info] Clearing ChromaDB directory at {args.db_dir}...")
            shutil.rmtree(args.db_dir)
        else:
            print("[info] ChromaDB directory does not exist — nothing to clear.")

        if not url:
            print("[info] No URL provided — exiting after clearing DB.")
            sys.exit(0)

    if not url:
        print("Error: URL is required unless you're only using --clear-db")
        sys.exit(1)
    ...
    seen_ids = set()      # <— track hashes we’ve already queued
    chunk_idx = 0
    ...
    for doc in crawl_results:
        ...
        for chunk in chunks:
            ...
            # skip if already in DB or already queued locally
            if document_exists(collection, doc_hash) or doc_hash in seen_ids:
                print(f"[skip] Duplicate chunk: {doc_hash}")
                continue
    ...
    add_documents_to_collection(collection, documents, metadatas, ids, batch_size=args.batch_size)
